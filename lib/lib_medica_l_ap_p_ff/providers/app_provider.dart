...
class TaxInfo {
  ...
  TaxInfo({required this.name, required this.rate, required this.amount});

  // Helper to convert to the map format your API expects
...
class AppProvider extends ChangeNotifier {
  ...
  void selectPlanType(PlanType type, int premium, String insurer) {
    _selectedPlanAmount = premium;
    _selectedPlanType = type;
    _selectedInsurer = insurer;
    _isPaymentFormVisible = false;

    _calculateTaxesAndTotal();

    print(
        'Selected Plan Type: $_selectedPlanType, Amount: $_selectedPlanAmount, Insurer: $_selectedInsurer');
    notifyListeners();
  }

  void _calculateTaxesAndTotal() {
    if (_selectedPlanAmount == null) {
      _calculatedTaxes = [];
      _totalPayable = 0;
      return;
    }

    final double premium = _selectedPlanAmount!.toDouble();
    const double pcfRate = 0.0025;
    const double itlRate = 0.0020;
    const double stampDuty = 40.00;

    final double pcfAmount = premium * pcfRate;
    final double itlAmount = premium * itlRate;

    _calculatedTaxes = [
      TaxInfo(name: 'PCF', rate: pcfRate, amount: pcfAmount),
      TaxInfo(name: 'ITL', rate: itlRate, amount: itlAmount),
      TaxInfo(
          name: 'STAMP DUTY',
          rate: 0,
          amount: stampDuty), // Rate is not percentage based
    ];

    _totalPayable = premium + pcfAmount + itlAmount + stampDuty;
  }
